...
virtual DWORD				   	clsAlertThread::ThreadFunction(void)
{
	...
	while (ThreadWait(dwWaitTime))
		...
		if (!MessageList.isEmpty())
			...
			clsMailClient						MailClient(__LocationConfig.GetSectionData("Mail"));
...
//	Options[clsIGMessage::IG_DL_OPTION_INVERTERHISTOINTERVAL] = 5;
			...
			switch (*pcArg++)
				...
				case 'o':
					...
					switch (cValue)
						{
						case 'h':
							Options[clsIGMessage::IG_DL_OPTION_INVERTERHISTOINTERVAL] = clsVariant(_chk_atoi(pcArg));
							break;
						case 's':
							Options[clsIGMessage::IG_DL_OPTION_SENSORHISTOINTERVAL] = clsVariant(_chk_atoi(pcArg));
							break;
						}
					}
					break;
	...
	if (bSendTestMail)
		...
		AlertUser(
			...
			String::Format("This mail was created by user '%s' on computer '%s'",(LPCTSTR)UserName(),(LPCTSTR)ComputerName()));
