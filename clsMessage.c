	...
	, _nExpectedBlockNumber(0)
{
	...
	if (!_bChecksumOk)
		{
		_chk_utildebug("bad checksum. Sizes %d %d, want: %02x",_nDataSize,nDataSize,_pData[_nDataSize]);
		for (int i = 0; i < _nDataSize; ++i)
			{
			_chk_utildebug(" is (%2d=%02x) %02x/%02x",i,_pData[i],crc(_pData,i),_pData[_nDataSize]);
			}
		}
...
clsIGMessage::clsIGMessage(enLocalNetID nFrom, enLocalNetID nTo, enCmd nCmd, BYTE* pData, unsigned nDataSize)
...
clsIGMessage::clsIGMessage(enLocalNetID nFrom, enLocalNetID nTo, enCmd nCmd, unsigned nDataSize, ...)
	: _pData(new BYTE[MaxBufferSize(nDataSize)])
	, _bChecksumOk(false)
	, _bComplete(true)
	, _nExpectedBlockNumber(0)
...
clsIGMessage&					clsIGMessage::operator=(const clsIGMessage& rhs)
{
	if (&rhs != this)
		{
...
bool							clsIGMessage::GetPacketsForCOMM(WCValSList<clsDataPacket*>& Packets) const
{
	if (_pData)
		{
		if (_nDataSize > 1000)
...
clsIGMessage&					clsIGMessage::operator+=(const clsIGMessage& rhs)
{
	if (rhs.nBlockNumber() != _nExpectedBlockNumber)
		{
		throw clsExcept("ERROR: bad block number 0x%02x (expected: 0x%02x)",rhs.nBlockNumber(),_nExpectedBlockNumber);
		}

	BYTE*	pNew = new BYTE[_nDataSize + rhs.PayloadSize()-1]; // remove block number...
	memcpy(pNew,_pData,_nDataSize);
	delete[] _pData;
	_pData = pNew;

	memcpy(_pData+_nDataSize,rhs.Payload()+1,rhs.PayloadSize()-1); // do not copy block number
...
void							clsIGMessage::AppendPayload(const BYTE* pData, unsigned nDataSize)
{
...
void							clsIGMessage::SetPayload(const BYTE* pData, unsigned nDataSize)
{
	if (3+nDataSize > _nDataSize)
		{
		BYTE*	pNew = new BYTE[3+nDataSize];
...
String							clsIGMessage::sCmd() const
{
	switch (nCmd())
		{
		case IGCMD_DEV_GETCONFIG:  			return("QRY_DEV_CONFIG");
		}
	if (IsInverter(nTo()))
		{
		switch (nCmd())
			{
			case IGCMD_INV_QUERYDISPLAY_CURRENT:	return("QRY_DATA_CUR");
			case IGCMD_SNS_QUERYDISPLAY_CURRENT:	return("QRY_DATA_CUR");
			default:						return(String::Format("UNK(0x%02x)",nCmd()));
			}
		}
	switch (nTo())
		{
		...
		case IGID_DATALOGGER:
			switch (nCmd())
				{
				case IGCMD_DEV_GETCONFIG:  		return("QRY_DEV_CONFIG");
				case IGCMD_GET_HISTORICALDATA:	return("QRY_DL_HIST");
//				case IGCMD_DATA_HISTORICALDATA:	return("HIST_DATA");
				case IGCMD_DL_GET_DATETIME:		return("QRY_DL_DTG");
				case IGCMD_DL_SET_DATETIME:		return("SET_DL_DTG");
				case IGCMD_DL_GET_OPTION:		return("QRY_DL_OPTION");
				case IGCMD_DL_SET_OPTION:		return("SET_DL_OPTION");
				case IGCMD_DL_GET_ID:			return("GET_DL_ID");
				default:						break;
				}
			break;
		case IGID_BROADCAST:
			switch (nCmd())
				{
				case IGCMD_DEVICELIST:			return("QRY_DEVLIST");
				default:						break;
				}
			break;
		}
...
String							clsIGMessage::AsString() const
{
	...
	s = String::Format("%-7s -> %-7s: %-8s: ",(LPCTSTR)sFrom(),(LPCTSTR)sTo(),(LPCTSTR)sCmd());
...
int								clsIGMessage::operator==(const clsIGMessage& rhs) const
{
