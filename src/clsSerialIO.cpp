...
void                            clsIOBase::IOLog(const char* pszFormat, ...)
{
    ...
    va_end(args);
#if defined(__TEST__)
    printf("%s %s: %s\n",(LPCTSTR)clsSYSTEMTIME(true).sTime(),(LPCTSTR)sDevice(),
           (LPCTSTR)s);
#else
    _chk_utildebug("%s %s: %s\n",(LPCTSTR)clsSYSTEMTIME(true).sTime(),
                   (LPCTSTR)sDevice(),(LPCTSTR)s);
#endif
...
void                            clsIOBase::IONtfyData(const BYTE* pData,
        UINT nSize)
{
#if 1
    String  s;
...
void                            clsIOBase::IOLogWrite(const BYTE* pData,
        UINT nSize)
{
#if 1
    String  s;
...
void                            clsRS232IO::IOReset()
{
    ...
    {
        throw clsExcept("ERROR: Cannot set COM port mask: %s",
...
bool                            clsRS232IO::IOOpen(const String& sPort)
{
    ...
    {
        throw clsExcept("ERROR: Cannot purge COM queue: %s",
                        (LPCTSTR)String::GetLastError());
    }
    ...
    {
        throw clsExcept("ERROR: Cannot get COM state: %s",
                        (LPCTSTR)String::GetLastError());
    }
    ...
    {
        throw clsExcept("ERROR: Cannot set COM state: %s",
                        (LPCTSTR)String::GetLastError());
    }
    ...
    {
        throw clsExcept("ERROR: Cannot set COM timeouts: %s",
                        (LPCTSTR)String::GetLastError());
    }
        ...
        if (bOverlappedIOPending)
        {
            ...
            switch (nEvent)
            {
                case -1:
                    ...
                    // timeout
                    IOLog("FATAL: cannot read from COMM port (heartbeat missing!): %s",
                          (LPCTSTR)String::GetLastError());
                    return(1);
#else
                    break;
...
bool                            clsRS232IO::IOWrite(BYTE* pData, int nSize)
{
    ...
    if (iRet == 0)
    {
        if (GetLastError() == ERROR_IO_PENDING)
        {
            ...
            {
                throw clsNotifyExcept("cannot write to COMM port: %s",
                                      (LPCTSTR)String::GetLastError());
            }
        }
        else
...
LATELOAD_FUNC_4(INVALID_HANDLE_VALUE,HDEVINFO,WINAPI,
                SetupDiGetClassDevs, CONST GUID*, LPCTSTR, HWND, DWORD)
LATELOAD_FUNC_1_VOID(WINAPI,
                     SetupDiDestroyDeviceInfoList, HDEVINFO)
LATELOAD_FUNC_5(FALSE,BOOL,WINAPI,
                SetupDiEnumDeviceInterfaces, HDEVINFO, SP_DEVINFO_DATA*, CONST GUID*, DWORD,
                SP_DEVICE_INTERFACE_DATA*)
LATELOAD_FUNC_6(FALSE,BOOL,WINAPI,
                SetupDiGetDeviceInterfaceDetail, HDEVINFO, SP_DEVICE_INTERFACE_DATA*,
                SP_DEVICE_INTERFACE_DETAIL_DATA*, DWORD, DWORD*, SP_DEVINFO_DATA*)
LATELOAD_FUNC_7(FALSE,BOOL,WINAPI,
                SetupDiGetDeviceRegistryProperty, HDEVINFO, SP_DEVINFO_DATA*, DWORD, DWORD*,
                PBYTE, DWORD, DWORD*)
LATELOAD_FUNC_2(FALSE,BOOL,WINAPI,
                SetupDiGetDeviceInfoListClass, HDEVINFO, GUID*)
LATELOAD_FUNC_3(FALSE,BOOL,WINAPI,
                SetupDiEnumDeviceInfo, HDEVINFO, DWORD, SP_DEVINFO_DATA*)
LATELOAD_FUNC_5(FALSE,BOOL,WINAPI,
                SetupDiGetDeviceInstanceId, HDEVINFO, SP_DEVINFO_DATA*, LPTSTR, DWORD, DWORD*)
LATELOAD_END_CLASS()
...
void                            clsUSBIO::NtfyData(const BYTE* pszData,
        UINT nSize)
{
    if (nSize > 0)
    {
...
virtual DWORD                   clsUSBIO::ThreadFunction(void)
{
    ...
    while (!bAborted)
    {
        ...
        if (::ReadFile(_Device.Handle(),szTmp,sizeof(szTmp),&dwBytesRead,&ovRead))
        {
            if (dwBytesRead == 0)
            {
                IOLog("USB port did close unexpectedly (EOF): %s",
                      (LPCTSTR)String::GetLastError());
                return(1);
            }
...
bool                            clsUSBIO::IOWrite(BYTE* pData, int nSize)
{
    ...
    if (iRet == 0)
    {
        if (GetLastError() == ERROR_IO_PENDING)
        {
            ...
            {
                throw clsNotifyExcept("cannot write to USB port: %s",
                                      (LPCTSTR)String::GetLastError());
            }
        }
        else
...
bool                            clsUSBIO::FindUSBDeviceOfGUID(String sDevice,
        clsIODevice& Device)
{
    GUID                        guidInterface;
    ...
    if (hDevInfo != INVALID_HANDLE_VALUE)
    {
                ...
                ++nInterface)
        {
            ...
            __SetupAPI.SetupDiGetDeviceInterfaceDetail(hDevInfo,&DeviceInterfaceData,NULL,
                    0,&dwSize,NULL);

            SP_DEVICE_INTERFACE_DETAIL_DATA*    pDetail =
                (SP_DEVICE_INTERFACE_DETAIL_DATA*)(new BYTE[dwSize]);
            pDetail->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);
            if (__SetupAPI.SetupDiGetDeviceInterfaceDetail(hDevInfo,&DeviceInterfaceData,
                    pDetail,dwSize,&dwSize,&DeviceInfoData))
            {
...
String                          clsUSBIO::GetDeviceDescProperty(
    HDEVINFO hDevInfo, SP_DEVINFO_DATA& DeviceInfoData, DWORD nSPDRP)
{
    String      sResult;
    DWORD       DataT;
    clsBuffer   buf(100);
...
LATELOAD_FUNC_3(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_ListDevices,
                void*, void*, DWORD)
LATELOAD_FUNC_2(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_Open,
                int, FT_HANDLE*)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,
                FT_GetLibraryVersion,       DWORD*)
LATELOAD_FUNC_4(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_Read,
                FT_HANDLE, void*, DWORD, LPDWORD)
LATELOAD_FUNC_4(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_Write,
                FT_HANDLE, void*, DWORD, LPDWORD)
LATELOAD_FUNC_2(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_Purge,
                FT_HANDLE, ULONG)
LATELOAD_FUNC_2(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_SetBaudRate,
                FT_HANDLE, ULONG)
LATELOAD_FUNC_4(FT_OTHER_ERROR,FT_STATUS,FTAPI,
                FT_SetDataCharacteristics,  FT_HANDLE, UCHAR, UCHAR, UCHAR)
LATELOAD_FUNC_4(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_SetFlowControl,
                FT_HANDLE, USHORT, UCHAR, UCHAR)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_ResetDevice,
                FT_HANDLE)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_ResetPort,
                FT_HANDLE)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_CyclePort,
                FT_HANDLE)
LATELOAD_FUNC_5(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_SetChars,
                FT_HANDLE, UCHAR, UCHAR, UCHAR, UCHAR)
LATELOAD_FUNC_3(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_SetTimeouts,
                FT_HANDLE, ULONG, ULONG)
LATELOAD_FUNC_2(FT_OTHER_ERROR,FT_STATUS,FTAPI,
                FT_GetDriverVersion,        FT_HANDLE, DWORD*)
LATELOAD_FUNC_6(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_GetDeviceInfo,
                FT_HANDLE, FT_DEVICE*, DWORD*, CHAR*, CHAR*, void*)
LATELOAD_FUNC_3(FT_OTHER_ERROR,FT_STATUS,FTAPI,
                FT_SetEventNotification,    FT_HANDLE, DWORD, HANDLE)
LATELOAD_FUNC_2(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_GetQueueStatus,
                FT_HANDLE, DWORD*)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_StopInTask,
                FT_HANDLE)
LATELOAD_FUNC_1(FT_OTHER_ERROR,FT_STATUS,FTAPI,             FT_RestartInTask,
                FT_HANDLE)


LATELOAD_FUNC_7(INVALID_HANDLE_VALUE,FT_HANDLE,FTAPI,       FT_W32_CreateFile,
                LPCSTR, DWORD, DWORD, SECURITY_ATTRIBUTES*, DWORD, DWORD, HANDLE)
LATELOAD_FUNC_1(FALSE,BOOL,FTAPI,                           FT_W32_CloseHandle,
                FT_HANDLE)
LATELOAD_FUNC_5(FALSE,BOOL,FTAPI,                           FT_W32_ReadFile,
                FT_HANDLE, void*, DWORD, DWORD*, OVERLAPPED*)
LATELOAD_FUNC_5(FALSE,BOOL,FTAPI,                           FT_W32_WriteFile,
                FT_HANDLE, void*, DWORD, DWORD*, OVERLAPPED*)
LATELOAD_FUNC_1(0,DWORD,FTAPI,
                FT_W32_GetLastError,        FT_HANDLE)
LATELOAD_FUNC_4(FALSE,BOOL,FTAPI,
                FT_W32_GetOverlappedResult,FT_HANDLE, OVERLAPPED*, DWORD*, BOOL)
LATELOAD_FUNC_2(FALSE,BOOL,FTAPI,
                FT_W32_GetCommState,        FT_HANDLE, FTDCB*)
LATELOAD_FUNC_2(FALSE,BOOL,FTAPI,
                FT_W32_SetCommState,        FT_HANDLE, FTDCB*)
LATELOAD_FUNC_2(FALSE,BOOL,FTAPI,                           FT_W32_SetCommMask,
                FT_HANDLE, DWORD)
LATELOAD_FUNC_2(FALSE,BOOL,FTAPI,
                FT_W32_SetCommTimeouts, FT_HANDLE, FTTIMEOUTS*)
LATELOAD_FUNC_3(FALSE,BOOL,FTAPI,
                FT_W32_WaitCommEvent,       FT_HANDLE, DWORD*, OVERLAPPED*)
LATELOAD_FUNC_1(FALSE,BOOL,FTAPI,                           FT_W32_CancelIo,
                FT_HANDLE)
LATELOAD_END_CLASS()

static  clsFTDI __FTDI;

clsFTDIIOBase::clsFTDIIOBase(const String& sType, IDataSink* pDataStore,
                             UINT nBaudrate)
...
bool                            clsFTDIIO::IOOpen(const String& sDevice)
{
    ...
    if (__FTDI.FT_ListDevices(&nDevices,NULL,FT_LIST_NUMBER_ONLY) == FT_OK)
    {
        ...
        for (int nDevice = 0; nDevice < nDevices; ++nDevice)
        {
            ...
            if (__FTDI.FT_ListDevices((void*)nDevice,buf,
                                      FT_LIST_BY_INDEX | FT_OPEN_BY_DESCRIPTION) == FT_OK)
            {
    ...
    if (_nDevice >= 0)
    {
        ...
        if (__FTDI.FT_GetDeviceInfo(_hDevice,&nType,&nID,szSerial,szDescription,
                                    NULL) == FT_OK)
        {
        ...
        __FTDI.FT_ResetDevice(_hDevice);
        __FTDI.FT_SetBaudRate(_hDevice, _nBaudrate);
        __FTDI.FT_SetDataCharacteristics(_hDevice, FT_BITS_8, FT_STOP_BITS_1,
                                         FT_PARITY_NONE);
        __FTDI.FT_SetFlowControl(_hDevice, FT_FLOW_RTS_CTS, 0, 0);
        __FTDI.FT_SetChars(_hDevice, 0, 0, 0, 0);
        __FTDI.FT_SetTimeouts(_hDevice, 100, 100);
...
virtual DWORD                   clsFTDIIO::ThreadFunction(void)
{
    ...
    if (__FTDI.FT_SetEventNotification(_hDevice,FT_EVENT_RXCHAR,
                                       evCharReceived.Handle()) != FT_OK)
    {
    ...
    while (ThreadWait(dwTimeout,nEvent,1,evCharReceived.Handle()))
    {
        ...
        if ((nFTDIState = __FTDI.FT_GetQueueStatus(_hDevice,
                          &dwBytesInQueue)) == FT_OK)
        {
            ...
            if (dwBytesInQueue > 0)
            {
                ...
                if (__FTDI.FT_Read(_hDevice, szTmp, min(dwBytesInQueue,sizeof(szTmp)),
                                   &dwBytesRead) == FT_OK)
                {
                    if (dwBytesRead > 0)
                    {
...
bool                            clsFTDIW32IO::IOOpen(const String& sPort)
{
    ...
    {
        throw clsExcept("ERROR: Cannot set COM port mask: %s",
                        (LPCTSTR)String::GetSysErrorString(__FTDI.FT_W32_GetLastError(_hDevice)));
    }
...
bool                            clsFTDIW32IO::IOWrite(BYTE* pData, int nSize)
{
    ...
    if (iRet == 0)
    {
        if (GetLastError() == ERROR_IO_PENDING)
        {
            ...
            {
                __FTDI.FT_ResetPort(_hDevice);
                throw clsNotifyExcept("cannot write to FTDI port (2): %s",
                                      (LPCTSTR)String::GetLastError());
            }
            iRet = 1;
        }
        ...
        {
            __FTDI.FT_ResetPort(_hDevice);
            throw clsNotifyExcept("cannot write to FTDI port: %s",
                                  (LPCTSTR)String::GetLastError());
        }
    }
