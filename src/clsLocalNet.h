...
public:
    ...
    clsIGMessage(enLocalNetID nFrom, enLocalNetID nTo, enCmd nCmd,
                 BYTE* pData = NULL, unsigned nDataSize = 0);
    clsIGMessage(enLocalNetID nFrom, enLocalNetID nTo, enCmd nCmd,
                 unsigned nDataSize, ...);
    ~clsIGMessage();
    ...
    bool                            GetPacketsForCOMM(WCValSList<clsDataPacket*>&
            Packets) const;
    ...
    void                            AppendPayload(const BYTE* pData,
            unsigned nDataSize);
    void                            AppendPayload(unsigned nDataSize, ...);
    void                            SetPayload(const BYTE* pData,
            unsigned nDataSize);
...
public:
    ...
    bool                            Get(WCValSList<clsIGMessage>& MsgList,
                                        unsigned nTimeout);
};
...
protected:
    ...
    bool                            GetMessage(clsIGMessage& Msg,
            unsigned nTimeout = 500);
    bool                            Query(const clsIGMessage& MsgQuery,
                                          const clsIGMessage& MsgFilter, clsIGMessage& MsgResponse,
                                          unsigned nTimeout = 500);
    bool                            Query(const clsIGMessage& MsgQuery,
                                          clsIGMessage& MsgResponse, unsigned nTimeout = 500)
    {
        return(Query(MsgQuery,MsgQuery,MsgResponse,nTimeout));
    }
    bool                            QueryMultiple(const clsIGMessage& MsgQuery,
            WCValSList<clsIGMessage>& MsgResponseList, int nMessages,
            unsigned nTimeout = 500);
public:
    clsIGInterface();
...
private:
    ...
    WCValSkipListDict<clsIGMessage::enDataloggerOption,clsVariant>
    _DataloggerOptions;
    ...
    bool                            GetNodeDevices(WCValSList<clsLocalNetID>&
            MissingDeviceList);
    bool                            QueryLoggerDateTime(clsSYSTEMTIME& ST);
    bool                            SetLoggerDateTime(const clsSYSTEMTIME& ST);
    bool                            QueryLoggerInverterHistoInterval(
        int& nMinutes);
    bool                            SetLoggerInverterHistoInterval(
        const int nMinutes);
    bool                            QueryLoggerSensorHistoInterval(int& nMinutes);
    bool                            SetLoggerSensorHistoInterval(
        const int nMinutes);
    bool                            QueryData(const clsLocalNetID& nID,
            enDisplaySelection nDisplaySelection, clsDataItemList* pDataList = NULL,
            String* pValue = NULL, String* psStateErrortext = NULL);
    bool                            QueryHistoricalData(const clsDate& Date,
            clsDeviceDataList* pDataList = NULL);
    bool                            QuerySensor(const clsLocalNetID& nID,
            clsDataItemList* pDataList = NULL, String* pValue = NULL);

    unsigned                        GetValueFromPayloadLE(int nBytes,
            const BYTE*& pPayload);
    unsigned                        GetValueFromPayloadBE(int nBytes,
            const BYTE*& pPayload);

    void                            OnState(BYTE nState,
                                            const String& sDescription);
public:
    clsLocalNet(enMode nMode,
                const WCValSkipListDict<clsIGMessage::enDataloggerOption,clsVariant>&
                DataloggerOptions);

    bool                            Open(String& sPort);
    enResult                        ProcessMessages(bool bActive,
            bool bFirstProcess);
...
inline clsLocalNet::enMode operator|(clsLocalNet::enMode a,
                                     clsLocalNet::enMode b)
{
