...
String                  sLocalNetID(enLocalNetID nID)
{
    ...
    switch (nID)
    {
        ...
        case IGID_DATALOGGER:
            return("LOGGER");
        case IGID_INTERFACECARD:
            return("INTERFACECARD");
        case IGID_INVALID:
            return(String::Format("INV(%02x)",nID));
        default:
            return(String::Format("UNK(%02x)",nID));
    }
}
...
String                  sLocalNetID(const clsLocalNetID& Device)
{
    if (IsInverter(Device.nID()))
    {
        ...
        {
            case 0xE3:
                return(String::Format("ID %02x(IG 4500-LV)",Device.nID()));
            case 0xE5:
                return(String::Format("ID %02x(IG 2500-LV)",Device.nID()));
            case 0xEA:
                return(String::Format("ID %02x(IG 5100)",Device.nID()));
            case 0xEB:
                return(String::Format("ID %02x(IG 4000)",Device.nID()));
            case 0xED:
                return(String::Format("ID %02x(IG 3000)",Device.nID()));
            case 0xEE:
                return(String::Format("ID %02x(IG 2000)",Device.nID()));
            case 0xF3:
                return(String::Format("ID %02x(IG 60/60-HV)",Device.nID()));
            case 0xF4:
                return(String::Format("ID %02x(IG 500)",Device.nID()));
            case 0xF5:
                return(String::Format("ID %02x(IG 400)",Device.nID()));
            case 0xF6:
                return(String::Format("ID %02x(IG 300)",Device.nID()));
            case 0xF9:
                return(String::Format("ID %02x(IG 60/60-HV)",Device.nID()));
            case 0xFA:
                return(String::Format("ID %02x(IG 40)",Device.nID()));
            case 0xFB:
                return(String::Format("ID %02x(IG 30 Dummy)",Device.nID()));
            case 0xFC:
                return(String::Format("ID %02x(IG 30)",Device.nID()));
            case 0xFD:
                return(String::Format("ID %02x(IG 20)",Device.nID()));
            case 0xFE:
                return(String::Format("ID %02x(IG 15)",Device.nID()));
            default:
                return(String::Format("UNK(%02x)",Device.nType()));
        }
    }
    else if (IsSensorcard(Device.nID()))
    {
        return(String::Format("ID %02x(SENSORCARD %d)",Device.nID(),
                              Device.nID()-IGID_SENSORCARD_FIRST+1));
    }
...
clsLocalNet::clsLocalNet(enMode nMode,
                         const WCValSkipListDict<clsIGMessage::enDataloggerOption,clsVariant>&
                         DataloggerOptions)
    : clsIGInterface()
    , _nMode(nMode)
    , _nStartDay(0)
    ...
    try
    {
        if (!sPort.IsEmpty())
        {
            ...
            if (bPortFound)
            {
                ...
                for (DWORD dwTimeout = GetTickCount() + 2000; GetTickCount() < dwTimeout
                        && !bUserAbortRequested(); )
                {
                    GetMessage(Msg,100);
        ...
        {
            for (int nPort = 1; nPort <= 255+3 && !bLoggerFound
                    && !bUserAbortRequested(); ++nPort)
            {
                bool        bPortFound(false);
...
unsigned                        clsLocalNet::GetValueFromPayloadLE(int nBytes,
        const BYTE*& pPayload)
{
    unsigned    nValue = 0;
...
unsigned                        clsLocalNet::GetValueFromPayloadBE(int nBytes,
        const BYTE*& pPayload)
{
    unsigned    nValue = 0;
...
String                          clsLocalNet::GetStateDescription(
    unsigned nState)
{
    struct scStateDescr
    {
...
bool                            clsLocalNet::GetNodeDevices(
    WCValSList<clsLocalNetID>& MissingDeviceList)
{
    ...
    if (bRes)
    {
        ...
        for (const BYTE* pData = pPayload;
                pData < pPayload + MsgResponse.PayloadSize(); )
        {
                ...
                if (Device.nID() != IGID_NODE_0E)
                {
                    ...
                    if (bRes)
                    {
                        ...
                        */
                        ...
                        sDescription += String::Format(" '%s'",(LPCTSTR)GetStringFromPayload(pPayload,
                                                       false).ATrim());
                        GetStringFromPayload(pPayload);
                        s.AddStrSeparated("\n ",sDescription);
                        int     nValues = GetValueFromPayloadLE(1,pPayload);
                        for (int nIndex = 0; nIndex < nValues
                                && (pPayload - MsgResponse.Payload()) < MsgResponse.PayloadSize(); ++nIndex)
                        {
                            unsigned    nType = GetValueFromPayloadLE(1,pPayload);
                            unsigned    nUnknown1 = GetValueFromPayloadLE(1,
                                                    pPayload); // usually 0, except for the first value...
                            unsigned    nUnknown2 = GetValueFromPayloadLE(1,
                                                    pPayload); // usually 0, except for the first value...
                            switch (nType)
                            {
                                case 0x03:
                                {
                                    ...
                                    s.AddStrSeparated("\n  ",String::Format("%s: ID %s",(const char*)sKey,
                                                                            (LPCTSTR)ID.AsStr()));
                                    if (sKey.Left(3) == "IG-")
                                ...
                                case 0x01:
                                {
                                    ...
                                    s.AddStrSeparated("\n  ",String::Format("%s: PCB %d.%d%c",(const char*)sKey,
                                                                            nVerHi,nVerLo,'A'+nVerRev));
                                }
                                break;
                                case 0x02:
                                {
                                    ...
                                    s.AddStrSeparated("\n  ",String::Format("%s: Software %d.%d",(const char*)sKey,
                                                                            nVerHi,nVerLo));
                                }
                                break;
                                case 0xff:
                                {
                                    ...
                                    s.AddStrSeparated("\n  ",String::Format("%s: %d/%d/%d",(const char*)sKey,
                                                                            nUnknown1,nUnknown2,nValue));
                                }
                                break;
                            }
                        }
                ...
                if (bInsertInList)
                {
                    ...
                    if (Device.nType() == IGTYPE_DATALOGGER)
                    {
                        ...
                        if (QueryLoggerInverterHistoInterval(nMinutes))
                        {
                            ...
                            if (_DataloggerOptions.contains(
                                        clsIGMessage::IG_DL_OPTION_INVERTERHISTOINTERVAL))
                            {
                                int     nNewMinutesNew = ((
                                                              _DataloggerOptions[clsIGMessage::IG_DL_OPTION_INVERTERHISTOINTERVAL].AsI4() +
                                                              4) / 5) * 5;
...
void                            clsLocalNet::OnState(BYTE nState,
...
clsLocalNet::enResult               clsLocalNet::ProcessMessages(bool bActive,
        bool bFirstProcess)
{
    clsIGMessage                    Msg;
    ...
    if (GetMessage(Msg,1000))
    {
        ...
        printf("unused: %02x %02x %02x %02x %02x\n",Msg.nFrom(),Msg.nTo(),Msg.nCmd(),
               Msg.PayloadSize(),Msg.Payload()[0]);
    ...
    if (bActive)
    {
        ...
        if (ST.wDay != _nStartDay)
        {
            ...
            for (INT nHour = max(0,(INT)floor(SI.dRise()-1));
                    nHour <= min(23,ceil(SI.dSet()+1)); ++nHour)
            {
        ...
        if ((_nMode & MODE_INSTALL) == 0)
        {
                    ...
                    _nHoursOfHistoUpdate.contains(ST.wHour))
            {
                ...
                for (WCValSListIter<clsOutputManager*> Destination(DestinationList);
                        ++Destination; )
                {
                        ...
                        (ST.wSecond % 10) == 0)
                {
                    ...
                    try
                    {
                        ...
                        printf("%02d:%02d:%02d: %s: %s\n",ST.wHour,ST.wMinute,ST.wSecond,
                               (LPCTSTR)sInstallation,(LPCTSTR)sValue);
                    }
                    catch(clsStateExcept& e)
                    {
                        printf("%02d:%02d:%02d: %s: %s\n",ST.wHour,ST.wMinute,ST.wSecond,
                               (LPCTSTR)sInstallation,(LPCTSTR)e.Text());
                    }
                }
            ...
            for (clsDeviceListIter Device(_ActiveDeviceList); ++Device; )
            {
                ...
                if (IsInverter(Device.key()))
                {
                    ...
                    try
                    {
                        ...
                        {
                            clsConfigOfInstallation Installation(__LocationConfig.Installation(
                                    Device.key()));
...
String  sValue(double dValue, int nDecs, enUnit nUnit)
{
    switch (nUnit)
    {
        ...
        case UNIT_RUNTIME_M:
        {
            ...
            return(String::Format("%d:%02d %s",(int)_round(dHours),(int)_round(dMinutes),
                                  pszUnit(nUnit)));
        }
    }
}
