...
void							clsIGInboundMessageList::Append(const clsIGMessage& Msg)
{
	_chk_debug(+1,">clsIGInboundMessageList(%#p)::Append (%d, %d, %d)",this,_List.contains(Msg),_List.entries(),_nMessages);
	_Crit.Enter();
...
bool							clsIGInboundMessageList::Get(clsIGMessage& Msg, unsigned nTimeout)
{
	...
	_chk_debug(-1,"<clsIGInboundMessageList::Get() -> %s",bOk ? (LPCTSTR)Msg.AsString() : "NO MESSAGE");
	return(bOk);
}

bool							clsIGInboundMessageList::Get(WCValSList<clsIGMessage>& MsgList, unsigned nTimeout)
{
	_chk_debug(+1,">clsIGInboundMessageList(%#p)::Get()",this);
	bool	bOk = false;
	...
	while (!bOk)
		...
		_chk_utildebug("...check empty=%d (%d,%d,%d)",_List.isEmpty(),_List.entries(),MsgList.entries(),_nMessages);
		while (!_List.isEmpty())
			{
/*
...
*/
	...
	_chk_debug(-1,"<clsIGInboundMessageList::Get() -> %d/%d",MsgList.entries(),_nMessages);
	return(bOk);
}
...
bool							clsIGInterface::GetMessage(clsIGMessage& Msg, unsigned nTimeout)
{
	...
	_chk_debug(-1,"<clsIGInterface::GetMessage -> %s",bRes ? (LPCTSTR)Msg.AsString() : "FAIL");
	return(bRes);
}

bool							clsIGInterface::QueryMultiple(const clsIGMessage& MsgQuery, WCValSList<clsIGMessage>& MsgResponseList, int nMessages, unsigned nTimeout)
{
...
bool							clsIGInterface::Query(const clsIGMessage& Query, const clsIGMessage& Filter, clsIGMessage& Msg, unsigned nTimeout)
{
	...
	_chk_debug(-1,"<clsIGInterface::Query -> %s",bRes ? (LPCTSTR)Msg.AsString() : "FAIL");
	return(bRes);
}
